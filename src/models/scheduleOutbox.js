...
export default class ScheduleOutbox extends DavCollection {
	...
	async freeBusyRequest(data) {
		...
		const responses = XPathUtility.select('/cl:schedule-response/cl:response', document, NS.resolve, XPathResult.ANY_TYPE, null)
		let responseNode

		while ((responseNode = responses.iterateNext()) !== null) {
			const recipient = XPathUtility.select('string(cl:recipient/d:href)', responseNode, NS.resolve, XPathResult.ANY_TYPE, null).stringValue
			const status = XPathUtility.select('string(cl:request-status)', responseNode, NS.resolve, XPathResult.ANY_TYPE, null).stringValue
			const calendarData = XPathUtility.select('string(cl:calendar-data)', responseNode, NS.resolve, XPathResult.ANY_TYPE, null).stringValue
