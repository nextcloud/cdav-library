	...
	it('should find all VCards and request only a set of VCard properties', () => {
		...
		return addressbook.findAllAndFilterBySimpleProperties(['EMAIL', 'UID', 'CATEGORIES']).then((res) => {
			...
			expect(request.report).toHaveBeenCalledWith('/foo/bar/folder/', { Depth: '1' },
				'<x0:addressbook-query xmlns:x0="urn:ietf:params:xml:ns:carddav"><x1:prop xmlns:x1="DAV:"><x1:getetag/><x1:getcontenttype/><x1:resourcetype/><x0:address-data><x0:prop name="EMAIL"/><x0:prop name="UID"/><x0:prop name="CATEGORIES"/></x0:address-data><x2:has-photo xmlns:x2="http://nextcloud.com/ns"/><x2:favorite xmlns:x2="http://nextcloud.com/ns"/></x1:prop></x0:addressbook-query>');
		}).catch(() => {
